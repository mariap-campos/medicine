import React, { useState, useEffect } from "react";
import AsyncStorage from "@react-native-async-storage/async-storage";
import { ScrollView, View, Text, Image } from "react-native";
import { Button, Divider } from "react-native-paper";
import { useNavigation } from "@react-navigation/native";
import { Header } from "../../components/Header";
import Pills from "../../assets/icons/medicine.png";
import Watch from "../../assets/icons/watch.png";
import { styles } from "./styles";
import { globalStyles } from "../../theme/globalStyles";
import { AddButton } from "../../components/AddButton";
import { COLORS } from "../../theme";
import { PillCard } from "../../components/PillCard";

export function Home() {
  const navigation = useNavigation();
  const [pills, setPills] = useState();

  const getList = async () => {
    try {
      const list = await AsyncStorage.getItem("PILLS");
      if (list) {
        setPills(JSON.parse(list));
      } else {
        setPills([]);
      }
    } catch (err) {
      console.debug(err);
    }
  };

  useEffect(() => {
    getList();
    const updateList = navigation.addListener("focus", () => {
      getList();
    });

    return updateList;
  }, []);

  if (!pills) {
    return <Text>Carregando...</Text>;
  }

  console.debug(pills);

  return (
    <>
      <Header />
      <AddButton />
      <ScrollView>
        <View style={styles.container}>
          <Text style={[globalStyles.title, styles.title]}>Meus Remédios</Text>
          <View>
            {pills.length === 0 && (
              <>
                <Text style={globalStyles.text}>Nada cadastrado ainda.</Text>
                <Text style={globalStyles.text}>
                  Clique no botão &quot; ou no botão abaixo para adicionar um
                  remédio
                </Text>
                <Button
                  color={COLORS.LIGHT_BLUE}
                  style={{ marginTop: 15 }}
                  mode="contained"
                  onPress={() => navigation.navigate("AddRemedy")}
                >
                  Adicionar
                </Button>
              </>
            )}
            {pills.map((pill) => (
              <PillCard pills={pills} pill={pill} updateList={getList} />
            ))}
          </View>

          <Text
            style={[
              globalStyles.title,
              styles.title,
              { marginTop: 40, marginBottom: 0 },
            ]}
          >
            Minha Rotina
          </Text>
          <View>
            <View>
              <View style={styles.time}>
                <Image source={Watch} style={styles.watchImage} />
                <Text style={globalStyles.title}>08:00</Text>
              </View>
              <View style={styles.cardSmall}>
                <View style={styles.logoBack}>
                  <Image source={Pills} style={styles.logo} />
                </View>
                <Text style={[globalStyles.text, styles.name]}>
                  {" "}
                  Metformina
                </Text>
              </View>
              <View style={styles.cardSmall}>
                <View style={styles.logoBack}>
                  <Image source={Pills} style={styles.logo} />
                </View>
                <Text style={[globalStyles.text, styles.name]}> As-med</Text>
              </View>
              <Divider
                style={{
                  marginTop: 8,
                  height: 0.8,
                  backgroundColor: COLORS.GRAY_PRIMARY,
                }}
              />
            </View>
            <View>
              <View style={styles.time}>
                <Image source={Watch} style={styles.watchImage} />
                <Text style={globalStyles.title}>08:00</Text>
              </View>
              <View style={styles.cardSmall}>
                <View style={styles.logoBack}>
                  <Image source={Pills} style={styles.logo} />
                </View>
                <Text style={[globalStyles.text, styles.name]}>
                  {" "}
                  Metformina
                </Text>
              </View>
              <View style={styles.cardSmall}>
                <View style={styles.logoBack}>
                  <Image source={Pills} style={styles.logo} />
                </View>
                <Text style={[globalStyles.text, styles.name]}> As-med</Text>
              </View>
              <Divider
                style={{
                  marginTop: 8,
                  height: 0.8,
                  backgroundColor: COLORS.GRAY_PRIMARY,
                }}
              />
            </View>
            <View>
              <View style={styles.time}>
                <Image source={Watch} style={styles.watchImage} />
                <Text style={globalStyles.title}>08:00</Text>
              </View>
              <View style={styles.cardSmall}>
                <View style={styles.logoBack}>
                  <Image source={Pills} style={styles.logo} />
                </View>
                <Text style={[globalStyles.text, styles.name]}>
                  {" "}
                  Metformina
                </Text>
              </View>
              <View style={styles.cardSmall}>
                <View style={styles.logoBack}>
                  <Image source={Pills} style={styles.logo} />
                </View>
                <Text style={[globalStyles.text, styles.name]}> As-med</Text>
              </View>
              <Divider
                style={{
                  marginTop: 8,
                  height: 0.8,
                  backgroundColor: COLORS.GRAY_PRIMARY,
                }}
              />
            </View>

            <View>
              <View style={styles.time}>
                <Image source={Watch} style={styles.watchImage} />
                <Text style={globalStyles.title}>12:00</Text>
              </View>
              <View style={styles.cardSmall}>
                <View style={styles.logoBack}>
                  <Image source={Pills} style={styles.logo} />
                </View>
                <Text style={[globalStyles.text, styles.name]}>
                  {" "}
                  Anlodipino
                </Text>
              </View>
              <Divider
                style={{
                  marginTop: 8,
                  height: 0.8,
                  backgroundColor: COLORS.GRAY_PRIMARY,
                }}
              />
            </View>
          </View>
        </View>
      </ScrollView>
    </>
  );
}
